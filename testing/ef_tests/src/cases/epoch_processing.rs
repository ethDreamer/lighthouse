...
impl<E: EthSpec> EpochTransition<E> for RewardsAndPenalties {
    fn run(state: &mut BeaconState<E>, spec: &ChainSpec) -> Result<(), EpochProcessingError> {
        match state {
            ...
            | BeaconState::Capella(_)
            | BeaconState::Deneb(_)
            | BeaconState::Whisk(_) => altair::process_rewards_and_penalties(
                state,
                &altair::ParticipationCache::new(state, spec).unwrap(),
                spec,
            ),
...
impl<E: EthSpec> EpochTransition<E> for SyncCommitteeUpdates {
    fn run(state: &mut BeaconState<E>, spec: &ChainSpec) -> Result<(), EpochProcessingError> {
        match state {
            ...
            | BeaconState::Capella(_)
            | BeaconState::Deneb(_)
            | BeaconState::Whisk(_) => altair::process_sync_committee_updates(state, spec),
        }
    }
}
...
impl<E: EthSpec> EpochTransition<E> for InactivityUpdates {
    fn run(state: &mut BeaconState<E>, spec: &ChainSpec) -> Result<(), EpochProcessingError> {
        match state {
            ...
            | BeaconState::Capella(_)
            | BeaconState::Deneb(_)
            | BeaconState::Whisk(_) => altair::process_inactivity_updates(
                state,
                &altair::ParticipationCache::new(state, spec).unwrap(),
                spec,
            ),
...
impl<E: EthSpec> EpochTransition<E> for ParticipationFlagUpdates {
    fn run(state: &mut BeaconState<E>, _: &ChainSpec) -> Result<(), EpochProcessingError> {
        match state {
            ...
            | BeaconState::Capella(_)
            | BeaconState::Deneb(_)
            | BeaconState::Whisk(_) => altair::process_participation_flag_updates(state),
        }
    }
}
