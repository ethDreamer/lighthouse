...
use state_processing::upgrade::{
    upgrade_to_altair, upgrade_to_bellatrix, upgrade_to_capella, upgrade_to_deneb, upgrade_to_whisk,
};
...
impl<E: EthSpec> Case for ForkTest<E> {
    ...
    fn result(&self, _case_index: usize, fork_name: ForkName) -> Result<(), Error> {
        ...
        let mut result = match fork_name {
            ...
            ForkName::Whisk => upgrade_to_whisk(&mut result_state, spec).map(|_| result_state),
        };
