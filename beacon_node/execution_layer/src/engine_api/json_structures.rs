...
impl<T: EthSpec> From<ExecutionPayload<T>> for JsonExecutionPayload<T> {
    fn from(execution_payload: ExecutionPayload<T>) -> Self {
        match execution_payload {
            ExecutionPayload::Merge(payload) => JsonExecutionPayload::V1(JsonExecutionPayloadV1 {
            ...
            ExecutionPayload::Capella(payload) => {
                JsonExecutionPayload::V2(JsonExecutionPayloadV2 {
                    parent_hash: payload.parent_hash,
                    fee_recipient: payload.fee_recipient,
                    state_root: payload.state_root,
                    receipts_root: payload.receipts_root,
            ...
            ExecutionPayload::Eip4844(payload) => {
                JsonExecutionPayload::V3(JsonExecutionPayloadV3 {
                    parent_hash: payload.parent_hash,
                    fee_recipient: payload.fee_recipient,
                    state_root: payload.state_root,
                    receipts_root: payload.receipts_root,
...
impl<T: EthSpec> From<JsonExecutionPayload<T>> for ExecutionPayload<T> {
    fn from(json_execution_payload: JsonExecutionPayload<T>) -> Self {
        match json_execution_payload {
            JsonExecutionPayload::V1(payload) => ExecutionPayload::Merge(ExecutionPayloadMerge {
                parent_hash: payload.parent_hash,
                fee_recipient: payload.fee_recipient,
                state_root: payload.state_root,
                receipts_root: payload.receipts_root,
            ...
            JsonExecutionPayload::V2(payload) => {
            ...
            JsonExecutionPayload::V3(payload) => {
