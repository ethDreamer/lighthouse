...
impl<E: EthSpec> SigpBlob<E> {
    pub fn random_valid<R: Rng>(rng: &mut R) -> Result<Self, String> {
        ...
        for i in 0..<E::Kzg as KzgPreset>::FIELD_ELEMENTS_PER_BLOB {
            let Some(byte) = blob_bytes.get_mut(i.checked_mul(<E::Kzg as KzgPreset>::BYTES_PER_FIELD_ELEMENT).ok_or("overflow".to_string())?)  else {
...
impl<'a, E: EthSpec> Arbitrary<'a> for SigpBlob<E> {
    fn arbitrary(u: &mut Unstructured<'a>) -> arbitrary::Result<Self> {
        ...
        for i in 0..<E::Kzg as KzgPreset>::FIELD_ELEMENTS_PER_BLOB {
            let Some(byte) = blob_bytes.get_mut(i.checked_mul(<E::Kzg as KzgPreset>::BYTES_PER_FIELD_ELEMENT).ok_or(arbitrary_error)?)  else {
