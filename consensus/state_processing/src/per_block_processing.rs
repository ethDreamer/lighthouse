...
pub fn apply_parent_execution_payload<E: EthSpec>(
    ...
    let parent_bid = state.latest_execution_payload_bid()?.clone_from_ref();
...
pub fn process_execution_payload_bid<E: EthSpec>(
    ...
    // [New in Heze:EIP8142] Verify that the committed payload can be disseminated as chunks
    if let Ok(payload_length) = bid.payload_length() {
        block_verify!(
            payload_length > 0,
            ExecutionPayloadBidInvalid::EmptyPayload.into()
        );
        let params = E::payload_chunk_params();
        block_verify!(
            params.chunk_size(payload_length as usize).is_ok(),
            ExecutionPayloadBidInvalid::PayloadTooLarge {
                payload_length,
                max_chunk_size: params.max_payload_chunk_size as u64,
            }
            .into()
        );
    }
    ...
    state.latest_execution_payload_bid_mut()?.assign(bid)?;
